...
Ms = ones(Int64, length(σs))*1001 # in time
...
for i in 1:σN
    ...
    Rs[i] = abs.(eval_mean.(t̄)) + interval(-1,1)*Cₚ*sqrt(σ*compute_b1(κ̃))/I"2"^I"1//4"*δs[i]
end

Rs = enlarge.(Rs)

if Rs[2] > Rs[1] && Rs[2] > Rs[3]
    println(L"$\mathcal{R}(\sigma_{SR}) > ", inf(Rs[2]))
end
